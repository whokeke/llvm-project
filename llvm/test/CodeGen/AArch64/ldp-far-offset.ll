...
; destination register lands in the LDP's SECOND destination slot (Rt2) and is
; also the adjusted base: `add Rt2, base, #imm; ldp Rt0, Rt2, [Rt2]` (base ==
; Rt2).  The backreferences `x[[TMP]]` for both the second destination and the
; base make this CHECK fail if a regression reintroduces a separate scratch.
...
; Regression guard for the GPR64 dest-reuse base-adjust path: when the load
; destination (Reg0) is itself IP0/IP1 (X16/X17), reusing it as the adjusted
; base reproduces the very linker-veneer hazard the separate-scratch path
; avoids.  The dest-reuse path must skip X16/X17 and fall through to the
; X9..X15 scavenger.  The scratch capture `(1[0-5]|[0-9])` cannot match 16/17.
...
; D1 loop-aware gating: loop-invariant base inside a loop should still
; allow base-adjust.
...
; D1 loop-aware gating: loop-variant base should NOT allow base-adjust.
; The base pointer changes each iteration (p+j), so base-adjust would be
; net negative (per-pair ADDXri stays in the loop).  Expect 2x ldr even
; with -aarch64-ldp-stp-base-adjust=1.
