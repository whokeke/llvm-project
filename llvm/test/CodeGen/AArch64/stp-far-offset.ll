...
; (X9..X15), so an exact `Reg == Reg0` compare misses the Xn-aliases-Wn
; case.  Pinning the two stored values to W9 and W8 forces Reg0 = W9: the
; buggy code would scavenge X9 (which overlaps W9) and emit
; `add x9, x0, #400; stp w9, w8, [x9]` -- the add clobbers W9 before the
; STP stores it.  The aliasing-aware skip must reject X9 here and pick a
; non-overlapping X10..X15.  The scratch capture `1[0-5]` cannot match 8/9.
